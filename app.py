"""
Gym Coach App - Multi-Exercise Trainer (Live RTL/LTR Localization — Fixed labels)
"""
...
# NEW: launcher لفصل التشغيل عن الواجهة
from modules.runtime.launcher import get_worker
...
class ExerciseScreen(QWidget):
    ...
    # ---------- MODIFIED: يستخدم runtime.launcher.get_worker ----------
    @Slot()
    def handle_start(self):
        ...
        # احصل على Worker من الـ runtime. إن لم يوجد تمرين مسجل، استعمل VideoWorker كـ fallback.
        w = get_worker(self.exercise_type, self.current_source, AppSettings)
        self.worker = w if w is not None else VideoWorker(self.current_source)

        # ربط الإشارات حسب ما يدعمه الـ worker
        if hasattr(self.worker, "metrics_ready"):
            self.worker.metrics_ready.connect(self.on_stats)
    ...
    def handle_stop(self):
        if self.worker and hasattr(self.worker, "isRunning") and self.worker.isRunning():
            # يوجد stop على BaseModelWorker وVideoWorker
    ...
    def on_stats(self, stats: dict):
        # متوقّع stats فيها "fps" وقد تحتوي "reps"/"angle"/"state"/"quality"
    ...
    def _update_metrics(self):
        ...
        # تحديثات إضافية إن وُجدت
        reps = self.last_metrics.get("reps")
        if reps is not None:
            self.lbl_reps.setText(str(int(reps)))
        angle = self.last_metrics.get("angle")
        if angle is not None:
            try:
                self.lbl_angle.setText(f"{float(angle):.1f}")
            except Exception:
                self.lbl_angle.setText(str(angle))
